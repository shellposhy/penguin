...
public class GobangChannel {
	...
	/**
	 * 正在进行中的棋局. String参数代表开局者（选手1）的userId ChessGame参数代表棋局
	 */
	public static final Map<String, ChessGame> chessPlaying = new ConcurrentHashMap<>();

	/**
	 * 对手，与正在进行的棋局Map配套使用. 第一个String代表player1, 第二个String代表player2
	 */
	...
	public void onConnect(final Session session) {
		...
		if (playing) {
			...
			if (chessGame == null) {
				...
				beanManager.getReference(ActivityMgmtService.class);
	...
	public void onMessage(final String message) throws JSONException {
		...
		case 1: // 聊天
			...
			break;
		case 2: // 落子
			ChessGame chessGame = chessPlaying.keySet().contains(player) ? chessPlaying.get(player)
					: chessPlaying.get(anti);
				...
				if (getAntiPlayer(player) == null) {
					...
					if (chessPlaying.get(player) != null) { // 说明玩家1断开了链接
						...
						if (!chessGame.isPlayState2()) {
							...
							// 由于玩家2先退出，补偿玩家1的积分
							final LatkeBeanManager beanManager = Lifecycle.getBeanManager();
							final ActivityMgmtService activityMgmtService = beanManager
									.getReference(ActivityMgmtService.class);
							activityMgmtService.collectGobang(chessGame.getPlayer1(),
									Pointtransfer.TRANSFER_SUM_C_ACTIVITY_GOBANG_START);
						} else {
							JSONObject sendText = new JSONObject();
							sendText.put("type", 6);
						...
						if (!chessGame.isPlayState1()) {
							...
							final ActivityMgmtService activityMgmtService = beanManager
									.getReference(ActivityMgmtService.class);
							activityMgmtService.collectGobang(chessGame.getPlayer2(),
									Pointtransfer.TRANSFER_SUM_C_ACTIVITY_GOBANG_START);
						} else {
							JSONObject sendText = new JSONObject();
							sendText.put("type", 6);
	...
	private void recoverGame(String userId, String userName, String antiUserId, ChessGame chessGame) {
		...
		sendText.put("type", 5);
		sendText.put("chess", chessGame.getChess());
		sendText.put("message", "【系统】：恢复棋盘，当前轮到玩家 ["
				+ (chessGame.getStep() == 1 ? chessGame.getName1() : chessGame.getName2()) + "] 落子");
		sendText.put("playerName", userName);
		sendText.put("player", userId);
		SESSIONS.get(userId).getAsyncRemote().sendText(sendText.toString());
		sendText = new JSONObject();
		sendText.put("type", 6);
		sendText.put("message", "【系统】：对手返回了棋局，当前轮到玩家 ["
				+ (chessGame.getStep() == 1 ? chessGame.getName1() : chessGame.getName2()) + "] 落子");
